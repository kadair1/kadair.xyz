...
(function () {
  ...
  // 0-180:    solar system orbits peacefully
  // 180-260:  spin faster, start pulling
  // 260-360:  frantic spin, collapse into black hole
  // 360-600:  black hole slowly shrinks to a dot (center of canvas)
  // 600+:     galaxy forms around the dot and persists forever

  var suckDone = false;
  var galaxyPhase = false;
  ...
  function draw() {
    ...
    if (!galaxyPhase) {
      ...
      if (!galaxyPhase) {
        ...
        if (bhScale > 0.15) {
          ...
          ctx.beginPath();
          ctx.arc(curBhX, curBhY, distR, 0, Math.PI * 2);
        ...
        for (var i = 0; i < bhParticles.length; i++) {
          ...
          ctx.beginPath();
          ctx.arc(px, py, pt.size * bhScale, 0, Math.PI * 2);
          ctx.fillStyle = "hsla(" + (15 + pt.dist * 2.5) + ", 85%, 55%, " + alpha + ")";
          ctx.fill();
        }
        ...
        bhGrad.addColorStop(0.6, "#050510");
        bhGrad.addColorStop(1, "rgba(20, 5, 50, 0.9)");
        ctx.beginPath();
        ctx.arc(curBhX, curBhY, Math.max(bhR, 1), 0, Math.PI * 2);
        ...
        if (bhR > 1.5) {
          ctx.beginPath();
          ctx.arc(curBhX, curBhY, bhR + 1.5, 0, Math.PI * 2);
        ...
        if (pull > 0.05 && solarScale > 0.05) {
          for (var s = 0; s < 8; s++) {
            ...
            ctx.quadraticCurveTo(
              ...
              curBhX + bhR * Math.cos(sa + 1),
              curBhY + bhR * Math.sin(sa + 1)
            );
        ...
        if (solarScale > 0.02) {
          ...
          ctx.scale(solarScale, solarScale);

          var sunR = 10 * (1 + 0.06 * Math.sin(t * 0.03));
          var sunGlow = ctx.createRadialGradient(0, 0, 0, 0, 0, sunR * 3);
          sunGlow.addColorStop(0, "rgba(255, 200, 50, 0.5)");
          sunGlow.addColorStop(0.5, "rgba(255, 180, 40, 0.15)");
          sunGlow.addColorStop(1, "rgba(255, 160, 30, 0)");
          ctx.beginPath();
          ctx.arc(0, 0, sunR * 3, 0, Math.PI * 2);
          ctx.fillStyle = sunGlow;
          ctx.fill();

          var sGrad = ctx.createRadialGradient(-2, -2, 0, 0, 0, sunR);
          sGrad.addColorStop(0, "#fff4b0");
          sGrad.addColorStop(0.4, "#ffd540");
          sGrad.addColorStop(1, "#f0a000");
          ctx.beginPath();
          ...
          eGrad.addColorStop(0, "#6db3f2");
          eGrad.addColorStop(0.5, "#3a8fd4");
          eGrad.addColorStop(1, "#1a5f9a");
          ctx.beginPath();
    ...
    if (galaxyPhase) {
      ...
      ctx.beginPath();
      ctx.arc(centerX, centerY, 12 * fadeIn, 0, Math.PI * 2);
      ctx.fillStyle = cGlow;
      ctx.fill();
      ...
      for (var i = 0; i < galaxyStars.length; i++) {
        ...
        ctx.beginPath();
        ctx.arc(sx, sy, s.size * starFadeIn, 0, Math.PI * 2);
